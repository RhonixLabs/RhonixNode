...
//  def resolveNoOp[M, T: Ordering](target: Set[M], meld: Meld[M, T]): ConflictResolution[T] =
//    ConflictResolution(meld.txsMap.view.filterKeys(target).values.flatten.toSet, Set())
  ...
  def computeRelationMap[D](directed: Boolean)(
    ...
    targetSet.iterator
      ...
      .foldLeft(mutable.Map.empty[D, Set[D]]) { case (acc, (target, source)) =>
  ...
  def computeBranches[D](target: Set[D], dependencyMap: Map[D, Set[D]]): Map[D, Set[D]] =
    dependencyMap.foldLeft(dependencyMap) { case (acc, (root, depending)) =>
