...
object AsyncCausalBuffer {
  ...
  def apply[F[_]: Async, T](): F[AsyncCausalBuffer[F, T]] =
    Ref.of[F, ConcurentDoublyLinkedDag[T]](ConcurentDoublyLinkedDag.empty[T]).flatMap(apply[F, T])

  def apply[F[_]: Async, T](stRef: Ref[F, ConcurentDoublyLinkedDag[T]]): F[AsyncCausalBuffer[F, T]] =
